...
// SendMessageStream sends a message through the Agent and returns a stream of events
// TODO: Instead of SendMessageOptions, take a domain.Message as input
func (a *Agent) SendMessageStreamOld(ctx context.Context, opts SendMessageOptions) AgentStream {
	eventsChan := make(chan events.Event)
	done := make(chan struct{})
	...
	go func() {
		...
		// TODO: If a message with role assistant is input, and that message has an ID and tool calls, then this is considered an approval. Call the tools and enter the agent loop.
			...
			case event, ok := <-llmStream.Events:
				...
				case *llm.MessageCompleteEvent:
					...
					if len(toolCalls) > 0 {
						...
						// TODO: Here, instead of returning and finishing the stream, continue the agent loop
					}
				case *events.ErrorEvent:
					eventsChan <- e
					return
